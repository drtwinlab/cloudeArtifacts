...
A Claude Code / Cowork plugin that turns Claude into an **autonomous UI/UX architect and Senior Frontend Engineer**. It runs a strict **4-phase design-to-code pipeline** for premium **React + Tailwind CSS** interfaces, driven by an always-on system prompt and four connected MCP servers.

The whole point: instead of one-shot "generate me a landing page" prompts that produce generic, average AI output, this plugin forces a disciplined flow — research and lock the art direction first, gather real assets second, write code that strictly inherits from the spec third, and validate the result in a real browser fourth.
...
MCP servers strictly phase by phase.
...
Claude automatically starts **Phase 1** (dialogue + reference selection + `DESIGN_SPEC.md`), and after your approval runs **Phases 2–4** autonomously.
